...
| G-7 — the copy test was inert, and its comment claimed the copy was load-bearing | **Comment corrected, and a redundant line removed — the test is still not falsifiable.** The verifier was right: the chain is composed eagerly, so the caller's slice is never retained and `slices.Clone` was doing nothing. The clone is gone; the function, its comment, the `Config.Middleware` doc, and the test now all say what actually holds. But the property is *structural* in Go — there is no implementation of eager composition under which the test could fail — so this is not a sensor and should not be counted as one. It is kept because it documents the guarantee at the call site. |
